...
def main(args=None):
    ...
    if parser.dataset == 'coco':
        ...
        dataset_train = CocoDataset(parser.coco_path, set_name='train128',
                                    transform=transforms.Compose([Normalizer(), Augmenter(), Resizer()]))
        dataset_val = CocoDataset(parser.coco_path, set_name='train128',
        ...
        dataset_train = DotaDataset(parser.coco_path, set_name='images',
                                    transform=transforms.Compose([Normalizer(), Augmenter(), Resizer()]),
                                    show=False)
            ...
            try:
                ...
                if torch.cuda.is_available():
                    classification_loss, regression_loss = retinanet(
                        [data['img'].cuda().float(), data['annot_h'], data['annot_r']])
                else:
                    classification_loss, regression_loss = retinanet(
                        [data['img'].float(), data['annot_h'], data['annot_r']])
    ...
    torch.save(retinanet, 'work_dir/model_final.pt')
